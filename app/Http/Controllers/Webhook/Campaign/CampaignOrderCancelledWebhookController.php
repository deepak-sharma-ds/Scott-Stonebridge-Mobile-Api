...
class CampaignOrderCancelledWebhookController extends Controller
{
    /**
     * Handle Shopify orders/cancelled and refunds/create webhooks for
     * Campaign Email Automation.
     *
     * - orders/cancelled → cancel ALL unsent campaign deliveries for the order.
     * - refunds/create   → cancel only the deliveries whose line item was
     *                       refunded (a partial refund of just the campaign
     *                       product stops that delivery; refunding other
     *                       items doesn't).
     *
     * SendCampaignEmailJob already guards on exact status, so a cancelled
     * delivery simply no-ops when its queued job eventually runs. Already-sent
     * deliveries cannot be unsent and are left untouched.
     */
    public function handle(Request $request): JsonResponse
    {
        ...
        if ($orderId === 0) {
            Log::channel('shopify_webhooks')->warning('Campaign cancel webhook received without order id', [
                'topic' => $topic,
        ...
        if ($webhookId) {
            ...
            if ($existing) {
                Log::channel('shopify_webhooks')->info('Duplicate campaign cancel webhook ignored', [
                    'webhook_id' => $webhookId,
                    'order_id' => $orderId,
                ]);
        ...
        try {
            ...
            Log::channel('shopify_webhooks')->error('Failed to persist campaign cancel webhook event', [
                'order_id' => $orderId,
                'error' => $e->getMessage(),
            ]);
        ...
        if (! (bool) config('campaign_email.cancel.enabled', true)) {
            Log::channel('shopify_webhooks')->info('Campaign cancel disabled; webhook ignored', [
                'event_id' => $event->id,
                'order_id' => $orderId,
            ]);
        ...
        $query = CampaignDelivery::where('shopify_order_id', $orderId)
            ->whereIn('status', [
        ...
        if ($isRefund) {
            ...
            if (empty($lineItemIds)) {
                Log::channel('shopify_webhooks')->info('Refund had no campaign line items to cancel', [
                    'event_id' => $event->id,
                    'order_id' => $orderId,
                ]);
        ...
        $cancelled = $query->get()->each(function (CampaignDelivery $delivery) {
            $delivery->forceFill(['status' => CampaignDelivery::STATUS_CANCELLED])->save();
        })->count();

        Log::channel('shopify_webhooks')->info('Campaign deliveries cancelled', [
            'event_id' => $event->id,
            'order_id' => $orderId,
            'topic' => $topic,
            'cancelled' => $cancelled,
