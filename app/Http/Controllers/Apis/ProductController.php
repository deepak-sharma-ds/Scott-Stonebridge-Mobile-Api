    ...
    public function getProductDetail($productId)
    {
        ...
        $products = array_map(function ($edge) {
            ...
            $product['variants'] = array_map(function ($variantEdge) {
                ...
                foreach (
                    $variant['inventoryItem']['inventoryLevels']['edges'] ?? [] as $level
                ) {
        ...
        try {
            ...
            if (! empty($filters['availability'])) {
                ...
                $parsed['products'] = array_values(array_filter($parsed['products'], function ($p) use ($availability) {
                    if (in_array('in_stock', $availability) && $p['availableForSale']) {
                        return true;
                    }
                    if (in_array('out_of_stock', $availability) && ! $p['availableForSale']) {
                        return true;
                    }
