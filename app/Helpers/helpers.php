...
if (! function_exists('isLoyaltyUserExists')) {
    ...
    {
        $userExists = UserLog::where('email', $email)->where('marketing_agreement', 1)->exists();
...
if (! function_exists('isOrderExists')) {
    ...
    {
        $orderExists = Order::where('order_number', $order_number)->exists();
...
{
    $order = Order::with('store')->where('order_number', $orderId)->first();
...
if (! function_exists('getPrescriptionData')) {
    ...
    {
        $prescriber_data = [];
        $prescriber_data = Prescriptions::select('prescriber_id', 'decision_status', 'clinical_reasoning')->where('order_id', $order_id)->first();
...
if (! function_exists('getOrderData')) {
    function getOrderData($order_id)
    {
        ...
        return Order::where('order_number', $order_id)->first();
    }
}
